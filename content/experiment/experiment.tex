...
\section{Experiments}
\label{sec:exp} 
An experimental suit composed of three experiments is designed to test the influence of the different blocks composing our framework in comparison to our previous work~\cite{Lemaintre2015miccaiOCT}.
These experiments are designed in order to investigate the effects of (i) optimal number of words, (ii) different pre-processing steps, and (iii) different classifiers.
Table~\ref{tab:experiment_summary} reports the experimentation in \cite{Lemaintre2015miccaiOCT} as a baseline and outlines the complementary experimentation here proposed.
The reminder of this section details the common configuration parameters across the experiments, while the detail explanations are presented in the following 
subsections. 
...
{\color{red} In all the experiments}, \ac{lbp} and \ac{lbptop} features are extracted using both \emph{local} and \emph{global}-mapping for different sampling points of 8, 16, and 24 for radius of 1, 2, and 3, respectively.
...
All the experiments are evaluated in terms \acf{se} and \acf{sp} using \ac{lopocv} strategy, in line with \cite{Lemaintre2015miccaiOCT}.
...
However, \ac{lopocv} strategy has been adapted despite this limitation due to the reduced size of the dataset.
...
%% Experiment intro
Experiment~\#1 is intended to find the optimal number of words (unlike \cite{Lemaintre2015miccaiOCT}) and its effect with respect to different configurations.

{\color{red}This experiment intends to find the optimal number of words and its effect with respect to different configurations, in contrary to \cite{Lemaintre2015miccaiOCT}, where the codebook size was arbitrarily set to $k = 32$.}
{\color{blue}This experiment explores several codebook sizes in variated conditions to find the optimal number of words in contrary to \cite{Lemaintre2015miccaiOCT}, where the codebook size was arbitrarily set to $k = 32$.}
...
%% Experiment procedure
Several pre-processing strategies are evaluated: (i) \nlm (NLM), (ii) a combination of \nlm and flatteningn (NLM+\f), and (iii) a combination of \nlm, flattening and aligning (NLM+\fal).
...
For this experiment, \ac{se} and \ac{sp} are complemented with \ac{acc} and \ac{f1} score (see Eg.\,\ref{eq:accf1}).
...
Complete set of \ac{acc} and \ac{f1} graph can be reproduced at \cite{Lemaitre2015}.
In order to illustrate the impact of the dictionary size, Fig.\,\ref{fig:RBOW} illustrates the \ac{acc} and \ac{f1} graph for a particular case\footnote{The shown example was chosen arbitrary and for the completer set please refer to \cite{Lemaitre2015}.}
In this figure, the performance of \emph{local}-\ac{lbp} features extracted from denoised and flatten slices while using three different radius (see Sect.\,\ref{subsec:feaext}, blue, green and red for radius of $\{1,2,3\}$, respectively) are illustrated. 
...
%% Experiment intro
This experiments explores the improvement associated with: (i) different pre-processing and (ii) using larger range of classifiers (i.e. linear and non-linear) for high represented features.
...
%Finally, the volumes are classified using $k$-\ac{nn}, \ac{rf}, \ac{gb}, and \ac{svm}.
The $k$-\ac{nn} classifier is trained by considering the 3 nearest neighbor.
...
Best results are achieve using \ac{svm} followed by \ac{rf}.
...
In terms of classifier \ac{rf} has a better performance than the others despite the fact that highest \ac{sp} is achieved using \ac{svm}.
