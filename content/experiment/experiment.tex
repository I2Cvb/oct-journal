...
The rest of this section details aspects of the experimentation and the design decisions that are consistent across all the experimentation, while subsections report different experiments.
...
All the experiments are evaluated in terms of \acf{se} and \ac{sp}, which are statistics driven from the confusion matrix (see Fig.~\ref{fig:CM}) as stated in \cref{eq:sesp}.
...
\emph{Experiment \#2 and \#3} (Sect.\,\ref{subsec:exp2}\,\&\,\ref{subsec:exp3})) studies the high-level feature extraction from the volumes using \bow.
...
Volumes are described using both low-level and high-level feature representation. For concordance with~\cite{Lemaintre2015miccaiOCT}, when using \bow the size of the coodebook is fixed to $32$ words.
...
%(see all configuration blocks shaded in Table~\ref{tab:tab3}).
