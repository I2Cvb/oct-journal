...
Lema\^itre\,\textit{et al.} propose a standard classification procedure to differentiate between \dme and normal \sdoct volumes.
The data is pre-processed using \nlm filtering. 
The volumes are mapped into a discrete set of structures namely local when these structures correspond to patches, or global when the structures corresponds to volume slices or the whole volume.
These structures are described in terms of \lbp, or \lbptop, and when necessary to represent each volume as a single feature vector, these structure sets are encoded either using histograms, \pca or \bow, to present the volumes to a \rf classifier.

The methodology was tested against Venhuizen\,\textit{et al.}~\cite{Venhuizen2015} using public and non-public datasets showing an improvement within the results, achieving a \ac{se} of $87.5\%$ and a \ac{sp} of $100\%$.
